...
import com.inerun.courier.adapter.WhAddParcelAdapter;
import com.inerun.courier.base.BaseFragment;
import com.inerun.courier.constant.UrlConstants;
import com.inerun.courier.data.ServiceResponse;
...
//        toolbar.setTitle(R.string.add_parcel_title);
    ...
    private void setData() {
        adapter = new WhAddParcelAdapter(activity());
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.rack_linear_layout:
                startActivityForResult(new Intent(activity(), CameraTestActivity.class), RACK_SCAN);
                break;
...
//            showSnackbar(R.string.saved_success);
            ...
            try {
                ...
                Map<String, String> params = DIRequestCreator.getInstance(activity()).getAddParcelMapParams(rack, parcellist);

                CourierApplication.serviceManager().postRequest(UrlConstants.URL_ADD, params, response_listener, response_errorlistener, ADDPARCEL);
    ...
    public void onDestroy() {
        super.onDestroy();
        CourierApplication.serviceManager().cancelAllRequest(ADDPARCEL);
    }
